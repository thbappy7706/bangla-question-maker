import { Document, Paragraph, TextRun, AlignmentType, BorderStyle, Packer, ImageRun, Footer } from 'docx';
...
export async function exportToDocx(qs: QuestionSet, questions: Question[], lang: Lang = 'bn') {
  ...
  const sectionHead = (title: string) => children.push(
    p([t(title, { bold: true, size: 26, color: '111111' })], { spacing: 300 })
  );
      ...
      if (s.mcqType === 'unified' && s.stem) {
        ...
        if (j > idx + 1) {
          ...
          if (!s.stem.includes('নিচের তথ্যের আলোকে') && !s.stem.includes('Based on the information')) {
            children.push(p(parseRuns(tr('mcq.unifiedInstruction', rangeStr), { bold: true, size: 24, color: '111111' }), { align: AlignmentType.CENTER, spacing: 200 }));
          }
        ...
        default: new Footer({
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: 'Generated by QuestionCraft',
                  size: 14,
                  color: 'D9D9D9',
                })
              ]
