...
export async function exportToPDF(qs: QuestionSet, questions: Question[], lang: Lang = 'bn') {
  ...
  pdf.text(t('export.headerTitle'), W / 2, 8, { align: 'center' });
  y = 20;
  ...
  const row1 = [qs.className && t('export.class', qs.className), qs.subjectName && t('export.subject', qs.subjectName)].filter(Boolean).join('   |   ');
  const row2 = [qs.fullMarks && t('export.fullMarks', qs.fullMarks), qs.duration && t('export.duration', qs.duration)].filter(Boolean).join('   |   ');
  ...
  if (qs.note) { pdf.setFontSize(8); pdf.setFont('helvetica', 'italic'); pdf.setTextColor('#666'); pdf.text(t('export.note', qs.note), W / 2, y, { align: 'center' }); y += 5; }
  ...
  if (srijonshil.length > 0) {
    sectionHeader(t('export.srijonshilSection'));
    srijonshil.forEach((q, i) => {
      ...
      if (s.uddipok) {
        ...
        const ul = pdf.splitTextToSize(t('export.uddipok', s.uddipok), CW - 6);
  ...
  if (songkhipto.length > 0) {
    sectionHeader(t('export.songkhiptoSection'));
  ...
  if (mcq.length > 0) {
    sectionHeader(t('export.mcqSection'));
    mcq.forEach((q, i) => {
