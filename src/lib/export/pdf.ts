...
function buildHtml(qs: QuestionSet, questions: Question[], t: ReturnType<typeof getT>): string {
  ...
  // Header bar
  html += `<div style="background:#059669;color:#fff;text-align:center;padding:14px 20px;font-weight:700;font-size:14px;letter-spacing:0.5px;">${escapeHtml(t('export.headerTitle'))}</div>`;
  ...
  // Divider
  html += `<hr style="border:none;border-top:2px solid #059669;margin:20px 0;" />`;
